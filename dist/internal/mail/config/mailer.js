...
const createBrevoMailer = ({ brevoApiKey, from }) => {
    ...
    return {
        sendMail: async (mailOptions) => {
            ...
            if (mailOptions.cc)
                payload.cc = parseAddress(mailOptions.cc);
            if (mailOptions.bcc)
                payload.bcc = parseAddress(mailOptions.bcc);
            if (mailOptions.text)
                payload.textContent = mailOptions.text;
            if (mailOptions.replyTo)
                payload.replyTo = { email: mailOptions.replyTo };
            if (mailOptions.attachments?.length) {
                payload.attachment = mailOptions.attachments.map((attachment) => ({
...
const createSmtpMailer = ({ nodemailer, host, port, secure, user, pass, connectionTimeout, greetingTimeout, socketTimeout, }) => {
    const buildTransportOptions = (smtpPort, smtpSecure) => ({
        host,
        port: smtpPort,
        secure: smtpSecure,
        auth: {
