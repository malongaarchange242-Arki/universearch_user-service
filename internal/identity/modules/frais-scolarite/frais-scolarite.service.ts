...
import { BulkUpdateFrais, FraisEntry, FraisResponse } from './frais-scolarite.schema';
...
export class FraisScolariteService {
   ...
   */
  async getFraisByUniversiteId(
    universite_id: string,
    filters?: { level?: string; pole?: string }
  ): Promise<FraisResponse[]> {
    try {
   ...
   */
  async getFraisById(id: string): Promise<FraisResponse | null> {
    try {
   ...
   */
  async deleteFraisByUniversiteId(universite_id: string): Promise<number> {
    try {
   ...
   */
  async hasFraisDefined(universite_id: string): Promise<boolean> {
    try {
      const { count, error } = await this.supabase
        .from('frais_scolarite')
